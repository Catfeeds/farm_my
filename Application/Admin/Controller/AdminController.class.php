...
class AdminController extends Controller {
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(empty($menus)){
            ...
            //--------+ 如果是内容管理的控制器，查询菜单时，带上type参数
            $controller_where = "url like '%{$controller}/".ACTION_NAME."%'";

            $controller_type=I('type');

            if ($controller=='Content'&&$controller_type==24){
                $controller_where = "url like '%{$controller}/".ACTION_NAME."?type=$controller_type%'";
            }
            //--------+

            $current = M('Menu')->where($controller_where)->field('id')->find();
    ...
    protected function stremail($str,$type=false,$error="含有非法字符请重输"){
        if($type){
            if($str==""){
                $this->error($error);
            }
    ...
    //用户资产行为日志
    protected function dailyRecord($rollback,$add_data){
        $property_m=M('property');
        $add_data['admin']=session('user_auth')['username'];
        $add_data['adminid']=session('user_auth')['uid'];
        $back=$property_m->add($add_data);
        if ($back==false){
            $rollback->rollback();
            $this->error('行为日志系统错误，请联系我们！');
        }
    }
