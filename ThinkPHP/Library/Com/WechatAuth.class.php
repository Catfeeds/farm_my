...
class WechatAuth {
	...
	/**
     * 通过网页登录时发送过来的code获取用户openid
     * @return openid
     */
    public function getOpenidByCode($code = null){
        $param = array(
            'appid'  => $this->appId,
    ...
    public function getJsticket($token){
        ...
        $token = json_decode($ticket, true);
        if(is_array($token)){
            $this->jsticket = $token['access_token'];
            return $token;
        } else {
     ...
     */
    public function __construct($appid, $secret, $token = null,$jsticket){
        if($appid && $secret){
            $this->appId     = $appid;
