    ...
    public function load($pFilename)
    {
        ...
        foreach ($this->sheets as $sheet) {
            ...
            if (!$this->readDataOnly && $this->drawingData) {
                ...
                // get all spforeachs in one long array, so they can be mapped to OBJ records
                $allSpforeachs = $escherWorksheet->getDgforeach()->getSpgrforeach()->getAllSpforeachs();
            }
...
//                echo '<br />';
                ...
                // the first shape foreach never has a corresponding OBJ record, hence $n + 1
                if (isset($allSpforeachs[$n + 1]) && is_object($allSpforeachs[$n + 1])) {
                    $spforeach = $allSpforeachs[$n + 1];

                    // we skip all spforeachs that are a part of a group shape since we cannot yet handle those
                    if ($spforeach->getNestingLevel() > 1) {
                        continue;
                    }

                    // calculate the width and height of the shape
                    list($startColumn, $startRow) = PHPExcel_Cell::coordinateFromString($spforeach->getStartCoordinates());
                    list($endColumn, $endRow) = PHPExcel_Cell::coordinateFromString($spforeach->getEndCoordinates());

                    $startOffsetX = $spforeach->getStartOffsetX();
                    $startOffsetY = $spforeach->getStartOffsetY();
                    $endOffsetX = $spforeach->getEndOffsetX();
                    $endOffsetY = $spforeach->getEndOffsetY();
...
//                            echo 'Picture Object<br />';
                            ...
                            $BSEindex = $spforeach->getOPT(0x0104);
                            $BSECollection = $escherWorkbook->getDggforeach()->getBstoreforeach()->getBSECollection();
                            ...
                            if ($blip = $BSE->getBlip()) {
                                ...
                                $drawing->setCoordinates($spforeach->getStartCoordinates());
                            }
                            break;
                        default:
                            // other object type
...
//        echo '<br />';
    ...
    private function readContinue()
    {
        ...
        //        0xF003 MsofbtSpgrforeach
        //        0xF004 MsofbtSpforeach
