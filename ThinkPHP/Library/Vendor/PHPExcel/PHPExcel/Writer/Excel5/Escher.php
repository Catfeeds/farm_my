...
/**
 * PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach
 *
 * @category   PHPExcel
 * @package    PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2015 PHPExcel (http://www.codeplex.com/PHPExcel)
    ...
    public function close()
    {
        ...
        switch (get_class($this->object)) {
            case 'PHPExcel_Shared_Escher':
                if ($dggforeach = $this->object->getDggforeach()) {
                    $writer = new PHPExcel_Writer_Excel5_Escher($dggforeach);
                    $this->data = $writer->close();
                } elseif ($dgforeach = $this->object->getDgforeach()) {
                    $writer = new PHPExcel_Writer_Excel5_Escher($dgforeach);
                    $this->data = $writer->close();
                    $this->spOffsets = $writer->getSpOffsets();
                    $this->spTypes = $writer->getSpTypes();
                }
                break;
            case 'PHPExcel_Shared_Escher_Dggforeach':
                // this is a foreach record
                ...
                // write the bstoreforeach
                if ($bstoreforeach = $this->object->getBstoreforeach()) {
                    $writer = new PHPExcel_Writer_Excel5_Escher($bstoreforeach);
                ...
                break;
            case 'PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach':
                // this is a foreach record
                ...
                $this->data = $header . $innerData;
                break;
            case 'PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE':
                // this is a semi-foreach record
                ...
                $this->data .= $data;
                break;
            case 'PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE_Blip':
                // this is an atom record

                // write the record
                switch ($this->object->getParent()->getBlipType()) {
                    case PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE::BLIPTYPE_JPEG:
                        // initialize
                        $innerData = '';
                    ...
                    case PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE::BLIPTYPE_PNG:
                        // initialize
                        $innerData = '';
                ...
                break;
            case 'PHPExcel_Shared_Escher_Dgforeach':
                // this is a foreach record
                ...
                $countShapes = count($this->object->getSpgrforeach()->getChildren());
                $innerData .= $header . pack('VV', $countShapes, $this->object->getLastSpId());
                //$innerData .= $header . pack('VV', 0, 0);

                // write the spgrforeach
                if ($spgrforeach = $this->object->getSpgrforeach()) {
                    $writer = new PHPExcel_Writer_Excel5_Escher($spgrforeach);
                    $innerData .= $writer->close();

                    // get the shape offsets relative to the spgrforeach record
                    $spOffsets = $writer->getSpOffsets();
                    $spTypes   = $writer->getSpTypes();
                    
                    // save the shape offsets relative to dgforeach
                    foreach ($spOffsets as & $spOffset) {
                        $spOffset += 24; // add length of dgforeach header data (8 bytes) plus dg data (16 bytes)
                    }
                ...
                $this->data = $header . $innerData;
                break;
            case 'PHPExcel_Shared_Escher_Dgforeach_Spgrforeach':
                // this is a foreach record
                ...
                // treat the inner data
                foreach ($this->object->getChildren() as $spforeach) {
                    $writer = new PHPExcel_Writer_Excel5_Escher($spforeach);
                    $spData = $writer->close();
                    $innerData .= $spData;
                ...
                $this->spTypes = $spTypes;
                break;
            case 'PHPExcel_Shared_Escher_Dgforeach_Spgrforeach_Spforeach':
                // initialize
                $data = '';
