...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    {
        // 1-based index to Bstoreforeach
        $blipIndex = 0;
        $lastReducedSpId = 0;
        $lastSpId = 0;
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            $dgforeach->setDgId($dgId);
            $escher->setDgforeach($dgforeach);

            // spgrforeach
            $spgrforeach = new PHPExcel_Shared_Escher_Dgforeach_Spgrforeach();
            $dgforeach->setSpgrforeach($spgrforeach);

            // add one shape which is the group shape
            $spforeach = new PHPExcel_Shared_Escher_Dgforeach_Spgrforeach_Spforeach();
            $spforeach->setSpgr(true);
            $spforeach->setSpType(0);
            $spforeach->setSpId(($sheet->getParent()->getIndex($sheet) + 1) << 10);
            $spgrforeach->addChild($spforeach);
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                // set the shape type
                $spforeach->setSpType(0x004B);
                // set the shape flag
                $spforeach->setSpFlag(0x02);

                // set the shape index (we combine 1-based sheet index and $countShapes to create unique shape index)
                $reducedSpId = $countShapes[$sheetIndex];
                $spId = $reducedSpId
                    | ($sheet->getParent()->getIndex($sheet) + 1) << 10;
                $spforeach->setSpId($spId);
                ...
                // set the BLIP index
                $spforeach->setOPT(0x4104, $blipIndex);
                ...
                $spforeach->setStartCoordinates($twoAnchor['startCoordinates']);
                $spforeach->setStartOffsetX($twoAnchor['startOffsetX']);
                $spforeach->setStartOffsetY($twoAnchor['startOffsetY']);
                $spforeach->setEndCoordinates($twoAnchor['endCoordinates']);
                $spforeach->setEndOffsetX($twoAnchor['endOffsetX']);
                $spforeach->setEndOffsetY($twoAnchor['endOffsetY']);
            ...
            if (!empty($filterRange)) {
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $spforeach = new PHPExcel_Shared_Escher_Dgforeach_Spgrforeach_Spforeach();
                    // set the shape type
                    $spforeach->setSpType(0x00C9);
                    // set the shape flag
                    $spforeach->setSpFlag(0x01);
                    ...
                    $spforeach->setOPT(0x007F, 0x01040104); // Protection -> fLockAgainstGrouping
                    $spforeach->setOPT(0x00BF, 0x00080008); // Text -> fFitTextToShape
                    $spforeach->setOPT(0x01BF, 0x00010000); // Fill Style -> fNoFillHitTest
                    $spforeach->setOPT(0x01FF, 0x00080000); // Line Style -> fNoLineDrawDash
                    $spforeach->setOPT(0x03BF, 0x000A0000); // Group Shape -> fPrint

                    // set coordinates and offsets, client anchor
                    $endCoordinates = PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::stringFromColumnIndex($iInc - 1));
                    $endCoordinates .= $rangeBounds[0][1] + 1;

                    $spforeach->setStartCoordinates($cDrawing);
                    $spforeach->setStartOffsetX(0);
                    $spforeach->setStartOffsetY(0);
                    $spforeach->setEndCoordinates($endCoordinates);
                    $spforeach->setEndOffsetX(0);
                    $spforeach->setEndOffsetY(0);
            ...
            $dgforeach->setLastSpId($lastSpId);
    ...
    private function buildWorkbookEscher()
    {
        ...
        $escher->setDggforeach($dggforeach);

        // set IDCLs (identifier clusters)
        $dggforeach->setIDCLs($this->IDCLs);
        ...
        $dggforeach->setSpIdMax($spIdMax + 1);
        $dggforeach->setCDgSaved($countDrawings);
        $dggforeach->setCSpSaved($totalCountShapes + $countDrawings); // total number of shapes incl. one group shapes per drawing

        // bstoreforeach
        $bstoreforeach = new PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach();
        $dggforeach->setBstoreforeach($bstoreforeach);
                ...
                if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    switch ($imageFormat) {
                        case 1: // GIF, not supported by BIFF8, we convert to PNG
                            $blipType = PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE::BLIPTYPE_PNG;
                            ob_start();
                            ...
                            break;
                        case 2: // JPEG
                            $blipType = PHPExcel_Shared_Escher_Dggforeach_Bstoreforeach_BSE::BLIPTYPE_JPEG;
